...
#boy = LineClient()
boy = LineClient(authToken='EIstK9kOCQOUCrZQo0u3.bzcvQGZc9U8c2k5YW0mbyW.Mlo5SVnhFFR+2R2VPNnkMVZpbqVCI1Z+ghzxK4F5yYY=')
...
def help2():
    ...
    helpMessage2 = "╔═══════════════════════╗" + "\n" + \
                  ...
                  "╠➣ " + key + "La\n" + \
                  "╠➣ " + key + "Lp\n" + \
                  "╠➣ ᴋᴇᴛɪᴋ「 ғʀᴇsʜ」ᴊɪᴋᴀ sᴜᴅᴀʜ\n╠➣ ᴍᴇɴɢɢᴜɴᴀᴋᴀɴ ᴄᴏᴍᴍᴀɴᴅ ᴅɪᴀᴛᴀs\n" + \
                  "╠═══════════════════════╗" + "\n" + \
                  "      📷sᴇʟғʙᴏᴛ ʙʏ ᴀʀᴇᴢᴀ📷" + "\n" + \
                  "╚═══════════════════════╝" 
    return helpMessage2
            ...
            if msg.toType == 2:
               ...
               if msg.contentType == 16:
                    if wait["Timeline"] == True:
                            ret_ = "[ᴅᴏɴᴇ ʟɪᴋᴇ]✓ ʟɪᴋᴇ ʙʏ : sᴇʟғʙᴏᴛ ʙʏ ᴊᴀᴄᴋʏ ᴀʀᴇᴢᴀ◉"
            ...
            if cctv['cyduk'][op.param1]==True:
                if op.param1 in cctv['point']:
                    ...
                    if Name in cctv['sidermem'][op.param1]:
                        ...
                        boy.sendMessage(op.param1, None, contentMetadata={"STKID":"171030368","STKPKGID":"14031","STKVER":"1"}, contentType=7)
                ...
                try:
                    ...
                    if msg_id in msg_dict1:
                        if msg_dict1[msg_id]["from"]:
                                ...
                                ret_ =  "⬇️「 sᴛɪᴄᴋᴇʀ ᴅɪʜᴀᴘᴜs 」⬇️\n"
                                ret_ += "• ɴᴀᴍᴀ ᴛᴇʀsᴀɴɢᴋᴀ : {}".format(str(Boy.displayName))
                                ret_ += "\n• ɴᴀᴍᴀ ɢʀᴜᴘ : {}".format(str(ginfo.name))
                                ret_ += "\n• ᴡᴀᴋᴛᴜ ɴɢɪʀɪᴍ : {}".format(str(cTime_to_datetime(msg_dict1[msg_id]["createdTime"])))
                 ...
                 if wait["detectMention"] == True:
                   ...
                   for mention in mentionees:
                        if mention ['M'] in Bots:
                           ...
                           boy.sendMessage(msg.to, None, contentMetadata={"STKID":"19200494","STKPKGID":"9242","STKVER":"2"}, contentType=7)
                           break
                        ...
                        if wait["Shared"] == True:
                            try:
                                ...
                                if msg.contentMetadata["serviceType"] == "GB":
                                    contact = boy.getContact(msg.contentMetadata[op.param3])
                                    auth = "\n╠ Penulis : {}".format(str(contact.displayName))
                                else:
                                    auth = "\n╠ Penulis : {}".format(str(msg.contentMetadata["serviceName"]))
                                purl = "\n╠ URL : {}".format(str(msg.contentMetadata["postEndUrl"]).replace("line://","https://me.me/R/"))
                                boy.likePost(purl[25:58], purl[66:], likeType=1001)
                                boy.sendMessage(purl[25:58], purl[66:], wait["comment"])
                                ret_ += auth
                                ret_ += purl
                                if "mediaOid" in msg.contentMetadata:
                                    ...
                                    if msg.contentMetadata["mediaType"] == "V":
                                        if msg.contentMetadata["serviceType"] == "GB":
                                            ourl = "\n╠ ᴏʙᴊᴇᴋ ᴜʀʟ : https://obs-us.line-apps.com/myhome/h/download.nhn?tid=612w&{}".format(str(msg.contentMetadata["mediaOid"]))
                                            murl = "\n╠ ᴍᴇᴅɪᴀ ᴜʀʟ : https://obs-us.line-apps.com/myhome/h/download.nhn?{}".format(str(msg.contentMetadata["mediaOid"]))
                                        else:
                                            ourl = "\n╠ ᴏʙᴊᴇᴋ ᴜʀʟ : https://obs-us.line-apps.com/myhome/h/download.nhn?tid=612w&{}".format(str(object_))
                                ...
                                if "stickerId" in msg.contentMetadata:
                                    stck = "\n╠ sᴛɪᴄᴋᴇʀ : https://me.me/R/shop/detail/{}".format(str(msg.contentMetadata["packageId"]))
                                    ret_ += stck
                                if "text" in msg.contentMetadata:
                                    text = "\n╠ ᴘᴇsᴀɴɴʏᴀ : {}".format(str(msg.contentMetadata["text"]))
                                    ret_ += text
                                ret_ += "\n╚══[ ✍sɪᴀᴘ ʟɪᴋᴇ & ᴋᴏᴍᴇɴ ʀᴇᴀᴅʏ ᴄᴏᴇɢ✍ ]"
                                boy.sendMessage(to, str(ret_))
                            except Exception as error:
                               print (error)
                        ...
                        if cmd == "uns off":
                          ...
                          if msg._from in admin:
                            ...
                            try:
                                ...
                                ret_ += "\n➣ 🚶ᴘᴇɴᴅɪʀɪ : {}".format(gPending)
                                ret_ += "\n➣ 👙ǫʀ : {}".format(gQr)
                                ret_ += "\n➣ 📠ᴛɪᴄᴋᴇᴛ : {}".format(gTicket)
                                ret_ += ""
                                boy.sendMessage(to, str(ret_))
                            except:
                                pass
...
#===========Hiburan============#
                          ...
                          if msg._from in admin:    
                            try:
                                search = msg.text.replace("lagu: ","")
                                r = requests.get("https://mnazria.herokuapp.com/api/joox?search={}".format(urllib.parse.quote(search)))
                                data = r.text
                                data = json.loads(data)
                                boy.sendAudioWithURL(to,data["mp3"])
                                boy.sendMessage(msg.to, "[⏳sᴜᴄᴄᴇss]√ 🔊sᴇʟᴀᴍᴀᴛ ᴍᴇɴᴅᴇɴɢᴀʀᴋᴀɴ ʟᴀɢᴜɴʏᴀ ʙᴏss 🎵🎶 ")
                                boy.sendMessage(msg.to, None, contentMetadata={"STKID":"51626501","STKPKGID":"11538","STKVER":"1"}, contentType=7)
                            except Exception as error:
...
#===========BOT UPDATE============#
                        elif cmd == "cek" or text.lower() == 'hai':
                          if msg._from in admin:
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in owner:
                                G = boy.getGroup(msg.to)
                                boy.sendText(msg.to, "╠➣ Bye bye fams "+str(G.name))
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                              try:
                                  tz = pytz.timezone("Asia/Jakarta")
                                  timeNow = datetime.now(tz=tz)
                                  boy.sendMessage(msg.to, "sɪᴅᴇʀ ᴅɪᴀᴋᴛɪғᴋᴀɴ\n\n📅ᴛᴀɴɢɢᴀʟ : "+ datetime.strftime(timeNow,'%Y-%m-%d')+"\n⌚ᴊᴀᴍ [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]")
                          ...
                          if wait["selfbot"] == True:
                           if msg._from in admin:
                              if msg.to in cctv['point']:
                                  ...
                                  boy.sendMessage(msg.to, "sɪᴅᴇʀ ᴅɪɴᴏɴᴀᴋᴛɪғᴋᴀɴ\n\n📅ᴛᴀɴɢɢᴀʟ : "+ datetime.strftime(timeNow,'%Y-%m-%d')+"\n⌚ᴊᴀᴍ [ "+ datetime.strftime(timeNow,'%H:%M:%S')+" ]")
                              else:
...
#===========COMMAND ON OFF============#
                          ...
                          if wait["selfbot"] == True:
                            if msg._from in admin:
                                ...
                                boy.sendMessage(msg.to,"ᴀᴜᴛᴏ ʀᴇᴘᴏɴ ᴅɪɴᴏɴᴀᴋᴛɪғᴋᴀɴ ")
